            ...
            for i, search_text in enumerate(search_texts):
                if search_text in content:
                    ...
                    if x is not None and y is not None:
                        c.drawString(x, y, new_texts[i])  # Draw the new text beside the search text
...
def get_text_position(content, search_text, new_text, x_offset):
    ...
    for line in lines:
        if search_text in line:
            x = line.find(search_text) * 20 + x_offset # Adjust the multipliers and offset for position
            y = lines.index(line) * 21.2  # Adjust the multiplier for vertical position
            return x, y
    return None, None
...
search_texts = ["334", "Joyce LABAYNO", "6:30-14:30 379Belen Ampoc MACKE"]  # List of texts to search for in the PDF
new_texts = ["New Text1", "New Text2", "New Text3"]  # Corresponding new texts to add if matched
